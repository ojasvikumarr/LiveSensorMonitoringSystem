...
/**
 * Simple JUnit tests for Consumer SensorReading model
...
class SensorReadingTest {
    ...
    @Test
    void testDataPersistenceScenarios() {
        // Test scenario: Data coming from Kafka message
        SensorReading kafkaReading = new SensorReading();
        kafkaReading.setSensorId("KAFKA_101");
        kafkaReading.setSensorType("TEMP_PRESSURE");
        kafkaReading.setTemperature(23.7);
        kafkaReading.setPressure(1012.3);
        kafkaReading.setTimestamp(LocalDateTime.now());
        kafkaReading.setLocation("Server-Room-A");

        assertNotNull(kafkaReading.getSensorId(), "Kafka reading should have sensor ID");
        assertTrue(kafkaReading.getTemperature() > 0, "Temperature should be valid");
        assertTrue(kafkaReading.getPressure() > 0, "Pressure should be valid");
        assertNotNull(kafkaReading.getTimestamp(), "Timestamp should be set");

        // Test scenario: Data stored in Redis
        SensorReading redisReading = new SensorReading(
            "REDIS_102", "HUMIDITY", 0.0, 45.2, LocalDateTime.now(), "Greenhouse-B"
        );
        
        assertEquals("REDIS_102", redisReading.getSensorId(), "Redis reading should maintain sensor ID");
        assertEquals("HUMIDITY", redisReading.getSensorType(), "Sensor type should be preserved");
        assertEquals(45.2, redisReading.getPressure(), 0.01, "Humidity value should be in pressure field");
    }
    ...
    void testTimeHandling() {
        ...
        SensorReading pastReading = new SensorReading(
            "PAST_101", "TEMP_PRESSURE", 20.0, 1000.0, pastTime, "Archive"
        );
        
        SensorReading futureReading = new SensorReading(
            "FUTURE_101", "TEMP_PRESSURE", 25.0, 1020.0, futureTime, "Forecast"
        );
        
        assertEquals(pastTime, pastReading.getTimestamp(), "Should handle past timestamps");
        assertEquals(futureTime, futureReading.getTimestamp(), "Should handle future timestamps");
        
        // Test ordering
        assertTrue(pastReading.getTimestamp().isBefore(futureReading.getTimestamp()), 
                  "Past reading should be before future reading");
    }

    @Test
    void testNullAndEmptyValues() {
        SensorReading nullReading = new SensorReading();
        
        // Test null values
        nullReading.setSensorId(null);
        nullReading.setSensorType(null);
        nullReading.setLocation(null);
        nullReading.setTimestamp(null);
        
        assertNull(nullReading.getSensorId(), "Should handle null sensor ID");
        assertNull(nullReading.getSensorType(), "Should handle null sensor type");
        assertNull(nullReading.getLocation(), "Should handle null location");
        assertNull(nullReading.getTimestamp(), "Should handle null timestamp");
        
        // Test empty string values
        nullReading.setSensorId("");
        nullReading.setSensorType("");
        nullReading.setLocation("");
        
        assertEquals("", nullReading.getSensorId(), "Should handle empty sensor ID");
        assertEquals("", nullReading.getSensorType(), "Should handle empty sensor type");
        assertEquals("", nullReading.getLocation(), "Should handle empty location");
    }
}
